...
void send_email(const char *from_email, const char *from_password, const char *to_email, const char *smtp_server, int smtp_port, const char *pdf_file_path)
{
    ...
    if (curl)
    {
        ...
        headers = curl_slist_append(headers, "This is an automated alert. Attached below is the Abnormality report containing the information which caused this alert to be generated.");
